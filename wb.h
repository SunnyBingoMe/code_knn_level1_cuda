...
// NVCC 5.0 does not support C++11 variadic templates. New CUDA 7 adds C++11 feature support, this part can be improved.
...
void wbTime_start(const wbTimeType timeType, const std::string timeMessage)
{
    ...
    std::cout << "             Started: " << timeMessage << "\n";
#else 
    std::cout << "TIMER_OK not defined.\n";
#endif
}
...
// OBS: dll will not return any value, it is void.
#if defined(_USRDLL) || defined(_WINDLL) // http://stackoverflow.com/questions/7697859/visual-studio-macro-for-checking-configuration-type-exe-dll
#define Check(stmt) do {                                                                        \
        cudaError_t err = stmt;                                                                 \
        if (err != cudaSuccess) {                                                               \
            wbLog(ERROR, "Failed to run: ", #stmt, ". CUDA error: ", cudaGetErrorString(err));  \
        }                                                                                       \
    } while(0)
#else
#define Check(stmt) do {                                                                        \
